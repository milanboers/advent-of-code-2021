...
findMap :: [Scanner] -> Scanner
findMap (x:xs) = findMap' xs x
  where
    findMap' :: [Scanner] -> Scanner -> Scanner
    findMap' [] prevS = prevS
    findMap' xs prevS = findMap' rest newS
      where
...
findAnswer scanners = length $ findMap scanners

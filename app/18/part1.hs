...
snailSum :: [Number] -> Number
snailSum (x:xs) = snailSum' xs x
  where
    snailSum' [x] acc = add acc x
    snailSum' (x:xs) acc = snailSum' xs (add acc x)
    snailSum' _ _ = error "Impossible"
snailSum _ = error "Empty list, no snailSum possible"
